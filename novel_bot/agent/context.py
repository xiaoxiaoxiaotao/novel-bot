...
{skills_summary}""")
        ...
        prompt_parts.append("3. **CRITICAL: DO NOT use 'read_file' to read CHARACTERS.md, WORLD.md, OUTLINE.md, SETTINGS.md, or STORY_SUMMARY.md** - these files are ALREADY included in your context above. Use them directly.")
        prompt_parts.append("4. **CRITICAL: DO NOT read drafts/*.md files directly** - use the '## RECENT CHAPTER SUMMARIES' section above which contains the short-term memory summaries.")
        prompt_parts.append("5. **CRITICAL: NEVER output long content directly to the user. ALWAYS use the 'write_file' tool to save content to files.**")
        ...
        prompt_parts.append("6. ALWAYS save your novel chapters in the 'drafts/' directory with format 'drafts/chapter_XX_Your_Title.md' (e.g., 'drafts/chapter_01_The_Beginning.md').")
        prompt_parts.append("7. **CRITICAL: When writing a chapter, ONLY call 'write_file' tool**. Do NOT call memorize_chapter_event in the same turn.")
        prompt_parts.append("   - Focus entirely on writing the best chapter content.")
        prompt_parts.append("   - In the NEXT turn (after user confirms), you will call memorize_chapter_event to save the chapter summary.")
        prompt_parts.append("8. **CRITICAL LONG-TERM MEMORY**: You MUST maintain 'STORY_SUMMARY.md' as a high-level plot synopsis of the ENTIRE story so far.")
        ...
        prompt_parts.append("9. **SMART QUESTION POLICY - MAIN ASK, SECONDARY AUTO**:")
        ...
        prompt_parts.append("10. **Writing Flow**: When user says 'write chapter X', immediately: 1) Use outline from context, 2) Write chapter, 3) Use 'write_file' tool to save to drafts/, 4) Update STORY_SUMMARY.md - ALL autonomously. Do NOT call memorize_chapter_event in the same turn.")
